...
class Goban:
    ...
    def __init__(self, input_filename):
        ...
        self.board_image = mpimg.imread(base_path + "/assets/board-bg.jpeg")
        self.black_image = mpimg.imread(base_path + "/assets/black.png")
        self.white_image = mpimg.imread(base_path + "/assets/white.png")
        ...
        ax = fig.add_subplot(111, facecolor="none", position=[0.1, 0.1, 0.8, 0.8])
        ax.tick_params(
            which="both",
            top=False,
            bottom=False,
    ...
    def plot_stone(self, xi, yi, image):
        assert (
            xi >= 1 and yi >= 1 and xi <= 19 and yi <= 19
        ), "Given coordinate out of range."
        ...
        ab = AnnotationBbox(
            im,
            (xi - 1 + fuzzy_offset_x, yi - 1 + fuzzy_offset_y),
            frameon=False,
            pad=0.0,
        )
    ...
    def render_board_background(self):
        board_padding = 0.6
        self.ax.imshow(self.board_image, extent=[-board_padding, 18.0 + board_padding, -board_padding, 18.0 + board_padding])
    ...
    def render_as_png(self, move_index):
        self.play_sgf_move(self.moves[move_index])
        self.fig.savefig(
            self.output_filename + "-" + "{:06}".format(move_index) + ".png"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        raise Exception("Please indicate an sgf file to be converted.")
    else:
